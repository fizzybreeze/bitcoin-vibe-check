...
`src/App.jsx` is ~1,450 lines — when you touch a card in it, extract that card
to `src/components/` in the same PR, since smaller diffs are what make review on
a phone practical. #22 tracks this, and it is done when nothing in `App.jsx` is
worth extracting — not at a line count.
...
| `Skeleton` † · `ChartTooltip` | Small presentational helpers (`DifficultyBar` is now private to `NetworkPulseCard`) |
