...
| `src/lib/alertRules.js` | What an alert is and when it fires — the metric registry, the crossing predicate, rule creation and the migration of rules stored under the old price-only shape. Pure, because §4.1b's server-side evaluator imports the same predicate and the two must not be able to disagree |
