...
//
// This array used to live in `vite.config.js` and be handed to VitePWA's
// `workbox` block. Under the `injectManifest` strategy there is no such block:
// the routes are registered by `src/sw.js`, which is the only production
// consumer. It stays a plain data array rather than a list of workbox calls so
// it can be asserted against without a service-worker global in sight.
